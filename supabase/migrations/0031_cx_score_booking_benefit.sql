-- CX Score membership benefit — the 5%/7%/10% CX Rent discount from
-- cx_score_levels, applied automatically to every booking based on the
-- renter's CURRENT CX Score at booking time. Unlike a `game_rewards` row
-- (a one-time coupon that gets consumed), this is an ongoing membership
-- perk: it is recomputed every time a price is quoted, never stored as
-- something to "use up", and it is capped in the database — the same
-- place the percentage is applied — so the frontend cannot influence
-- either the rate or the €-cap it's held to.
--
-- This sits alongside the existing `reward_id` discount (from
-- game_rewards) rather than replacing it — a player who already holds an
-- unclaimed Drive Challenge coupon can still redeem it; the racing game
-- that produced new ones has been removed from the app's navigation, so
-- that source now only drains, it never refills.

alter table public.bookings
  add column cx_discount_amount numeric not null default 0;

-- ============================================================
-- quote_booking — adds `cx_discount` to the returned breakdown so the
-- frontend can show "Membership discount (CX Member, 5%): -€25" as its
-- own line rather than folding it silently into the reward discount.
-- ============================================================
...
-- ============================================================
-- prepare_booking — same CX benefit, applied on the row that's actually
-- inserted, into the new cx_discount_amount column.
-- ============================================================
...
-- ============================================================
-- recompute_booking_on_date_change — same benefit, re-evaluated whenever
-- a booking's dates change (the CX Score itself is read fresh each time,
-- consistent with "never stored as something to use up").
-- ============================================================
