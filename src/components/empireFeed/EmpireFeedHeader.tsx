 ...
 *  a notifications entry (reusing the site's own existing Notifications
 *  page — not a second notification system), and a clear way back to CX
 *  Rent. `onExit`'s destination is fixed (`/dashboard` signed-in, `/`
 *  signed-out), not `navigate(-1)` — same predictable-exit convention
 *  the old game shell used, regardless of how the visitor arrived at
 *  `/empire`. Stays translucent at the very top of the feed and gains a
 *  touch more opacity once scrolled — the same `scrolled` pattern
 *  Navbar.tsx's PublicNavbar already uses — so it overlays content
 *  rather than reading as a flat website navbar. */
